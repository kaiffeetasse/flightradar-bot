...
def track(update, context):
    ...
    if not aircraft_registration.replace("-", "").replace("*", "").isalnum() or len(aircraft_registration) > 10:
    ...
    if tracking:
        ...
        if "*" in aircraft_registration:
            aircrafts = flightradar24_api.search_by_query(aircraft_registration.replace("*", ""))
            for aircraft in aircrafts:
                aircraft_registrations.append(aircraft["id"])
        else:
            aircraft_registrations.append(aircraft_registration)

        for aircraft_registration in aircraft_registrations:
